...
(function(app) {
    ...
    async function fetchEmployees() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to fetch employees: ${response.status} ${response.statusText}${errorText ? ` - ${errorText}` : ''}`);
            }
            const data = await response.json();
            console.log('📋 ADMIN: Received employee data:', data);
            
            if (!Array.isArray(data)) {
                throw new Error('Invalid employee data received: expected array');
            }
            ...
            // Ensure employees array is initialized even on error
            employees = [];
            renderEmployees();
        }
    }
    ...
    function renderEmployees() {
        ...
        if (!Array.isArray(employees) || employees.length === 0) {
            ...
            employeeTable.innerHTML = '<tr><td colspan="5" class="text-center">No employees found</td></tr>';
            return;
        }
        ...
        employees.forEach(employee => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <button class="btn btn-sm btn-primary" onclick="LaundryAdmin.editEmployee('${employee._id}')">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-sm btn-danger" onclick="LaundryAdmin.deleteEmployee('${employee._id}')">
    ...
    function editEmployee(id) {
        console.log('✏️ ADMIN: Loading employee for editing:', id);
        fetch(`/api/employees/${id}`)
            ...
            .then(employee => {
                console.log('📋 ADMIN: Employee data loaded:', employee);
                ...
                document.getElementById('role').value = employee.role || 'staff';
                document.getElementById('status').value = employee.status || 'active';
                
                // Update button text
                submitBtn.textContent = 'Update Employee';
                
                console.log('✅ ADMIN: Form populated for editing');
            })
            .catch(error => {
                console.error('❌ ADMIN: Error loading employee for edit:', error);
                showMessage('Error loading employee data', true);
            });
    }

    // Delete employee
    function deleteEmployee(id) {
        console.log('🗑️ ADMIN: Attempting to delete employee:', id);
    ...
    form.addEventListener('submit', (e) => {
        e.preventDefault();
        console.log('📝 ADMIN: Handling form submission');
        ...
        console.log(`📤 ADMIN: ${method} request to ${url}`, employeeData);
        fetch(url, {
            method,
            headers: {
                'Content-Type': 'application/json'
    ...
    // Expose necessary functions to window.LaundryAdmin
    Object.assign(LaundryAdmin, {
        deleteEmployee,
        editEmployee,
        selectEmployee
    });
    ...
    async function fetchTimesheetData() {
        ...
        if (!selectedEmployee) {
            ...
            timesheetTable.innerHTML = '<tr><td colspan="5" class="text-center">Please select an employee</td></tr>';
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`Failed to fetch timesheet data: ${response.status} ${response.statusText}${errorText ? ` - ${errorText}` : ''}`);
            }
            ...
            if (!Array.isArray(data)) {
                throw new Error('Invalid timesheet data received: expected array');
            }
            ...
            showMessage(`Error loading timesheet data: ${error.message}`, true);
            timesheetData = [];
            timesheetTable.innerHTML = '<tr><td colspan="5" class="text-center text-danger">Failed to load timesheet data: ' + error.message + '</td></tr>';
        }
    }
