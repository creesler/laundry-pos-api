...
(function(app) {
    ...
    async function fetchEmployees() {
        ...
        try {
            ...
            console.log('📥 ADMIN: Employee API response status:', response.status);
            const data = await response.json();
            console.log('📋 ADMIN: Received employee data:', data);
            employees = data;
            
            // Add test timesheet data for each employee
            for (const employee of employees) {
                await addTestTimesheetData(employee.name);
            }
    ...
    function renderTimesheet() {
        console.log('🔄 ADMIN: Rendering timesheet');
        timesheetTable.innerHTML = '';
        let totalHours = 0;

        if (!Array.isArray(timesheetData)) {
            console.log('⚠️ ADMIN: No timesheet data to display');
            timesheetTable.innerHTML = '<tr><td colspan="5">No timesheet data available</td></tr>';
            return;
        }
        ...
        console.log('✅ ADMIN: Timesheet rendered successfully');
    }
