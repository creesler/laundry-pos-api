...
(function(app) {
    ...
    async function fetchEmployees() {
        console.log('🔄 Starting employee fetch from MongoDB...');
        try {
            console.log(`📡 Making API request to: ${API_URL}/employees`);
            ...
            const data = await response.json();
            console.log('📋 Raw employee data from MongoDB:', JSON.stringify(data, null, 2));
            
            // Filter only active employees
            const activeEmployees = data.filter(emp => emp.status === 'active');
            console.log('✅ Active employees filtered:', JSON.stringify(activeEmployees, null, 2));
            console.log(`📊 Stats: Total employees: ${data.length}, Active: ${activeEmployees.length}, Inactive: ${data.length - activeEmployees.length}`);
            
            // Update global employees array
            employees = activeEmployees;
            console.log('🔄 Updated global employees array:', JSON.stringify(employees, null, 2));
            ...
            console.log('🎨 Rendering employee tabs...');
            renderEmployeeTabs();
            
            // Check selected employee status
            console.log('🔍 Current selected employee:', selectedEmployee);
            if (!activeEmployees.find(emp => emp.name === selectedEmployee)) {
                console.log('⚠️ Selected employee not found in active list');
                if (activeEmployees.length > 0) {
                    console.log(`✨ Selecting first available employee: ${activeEmployees[0].name}`);
            ...
            if (cachedEmployees) {
                console.log('📋 Found cached employees:', cachedEmployees);
                employees = JSON.parse(cachedEmployees);
                selectedEmployee = localStorage.getItem('selectedEmployee') || null;
                console.log('🔄 Restored from cache - Employees:', employees.length, 'Selected:', selectedEmployee);
                renderEmployees();
                renderEmployeeTabs();
            } else {
                console.log('❌ No cached employees found');
            }
        }
    }
    ...
    function init() {
        ...
        // Initialize timesheet dates
        console.log('📅 Initializing timesheet dates...');
    ...
    function renderEmployeeTabs() {
        console.log('🎨 Starting renderEmployeeTabs...');
        console.log('📋 Current employees array:', JSON.stringify(employees, null, 2));
        console.log('🎯 Selected employee:', selectedEmployee);
        
        employeeTabs.innerHTML = '';
        
        if (!employees || employees.length === 0) {
            console.log('⚠️ No employees to render tabs for');
            return;
        }

        let tabsCreated = 0;
        employees.forEach(employee => {
            console.log(`👤 Processing employee: ${employee.name}, Status: ${employee.status}`);
            // Only create tabs for active employees
            if (employee.status === 'active') {
                ...
                tabsCreated++;
                console.log(`✅ Created tab for: ${employee.name} (Active: ${employee.name === selectedEmployee})`);
            } else {
                console.log(`⏩ Skipped inactive employee: ${employee.name}`);
            }
        });
        
        console.log(`🏁 Finished creating tabs. Total created: ${tabsCreated}`);
    }

    // Select employee and update timesheet
    function selectEmployee(employeeName) {
        console.log('🎯 Attempting to select employee:', employeeName);
        console.log('📋 Current employees:', JSON.stringify(employees, null, 2));
        
        // Check if employee exists and is active
        const employee = employees.find(emp => emp.name === employeeName && emp.status === 'active');
        ...
        if (employee) {
            ...
            console.log('⚠️ Selected employee not found or inactive:', employeeName);
            // Select first active employee instead
            if (employees.length > 0) {
                const firstActive = employees.find(emp => emp.status === 'active');
                if (firstActive) {
                    console.log('🔄 Selecting first active employee:', firstActive.name);
                    selectEmployee(firstActive.name);
                } else {
                    console.log('❌ No active employees available');
                    selectedEmployee = null;
    ...
    // Save timesheet to server
    async function saveToServer() {
        console.log('🚀 Starting saveToServer process...');
        
        try {
            console.log('🔍 First checking MongoDB for employee updates...');
            // Check for employee updates first
            await fetchEmployees();
            console.log('✅ Employee list synchronized with MongoDB');

            if (!selectedEmployee) {
                console.log('❌ No employee selected, cannot save');
                showMessage('Please select an employee first', true);
                return;
            }

            console.log('📤 Preparing timesheet data for:', selectedEmployee);
            const timesheet = getTimesheetData();
            console.log('📋 Timesheet data:', JSON.stringify(timesheet, null, 2));

            console.log(`📡 Sending data to: ${API_URL}/timesheet`);
            ...
            if (response.ok) {
                ...
                console.log('🧹 Clearing timesheet form...');
                clearForm();
            } else {
            ...
            showMessage('Error saving timesheet', true);
        }
    }
})(window.LaundryAdmin); 