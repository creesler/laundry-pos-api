...
(function(app) {
    ...
    function renderEmployees() {
        ...
        employees.forEach(employee => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <button class="action-btn edit-btn" onclick="LaundryAdmin.editEmployee('${employee._id}')">Edit</button>
                    <button class="action-btn delete-btn" onclick="LaundryAdmin.deleteEmployee('${employee._id}')">Delete</button>
                    <button class="action-btn remove-btn" onclick="LaundryAdmin.removeEmployee('${employee._id}')">Remove</button>
                </td>
            `;
        });
    }
    ...
    // Expose necessary functions to the global LaundryAdmin object
    app.editEmployee = editEmployee;
    app.deleteEmployee = deleteEmployee;
    app.removeEmployee = removeEmployee;
    app.selectEmployee = selectEmployee;
    app.fetchEmployees = fetchEmployees;
    app.showMessage = showMessage;
    app.clearForm = clearForm;
    app.refreshData = refreshData;
    app.fetchTimesheetData = fetchTimesheetData;

    // Initialize when DOM is loaded
    document.addEventListener('DOMContentLoaded', () => {
        initializeTimesheetDates();
        fetchEmployees(); // Changed from fetchTimesheetData to fetchEmployees
    ...
    function renderTimesheet() {
        ...
        timesheetData.forEach(entry => {
            ...
            row.innerHTML = `
                ...
                <td>${formatDateTime(entry.clockIn, 'time')}</td>
                <td>${formatDateTime(entry.clockOut, 'time')}</td>
                <td>${formatDuration(entry.duration)}</td>
                <td>
                    <span class="status-badge ${entry.status === 'completed' ? 'status-completed' : 'status-pending'}">
    ...
    document.querySelector('.filter-btn').addEventListener('click', fetchTimesheetData);
