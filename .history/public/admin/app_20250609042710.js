    ...
    function updateInventoryChart(data, type = 'stock') {
        ...
        if (type === 'stock') {
            ...
            inventoryChart.data.datasets = [
                ...
                {
                    label: 'Min Stock',
                    data: data.map(item => item.minStock),
    ...
    function updateInventoryTable(data) {
        ...
        data.forEach(item => {
            ...
            const status = item.currentStock <= item.minStock ? 'Low Stock' : 
                          item.currentStock >= item.maxStock ? 'Overstocked' : 'Normal';
            const statusClass = status === 'Low Stock' ? 'text-danger' :
                              status === 'Overstocked' ? 'text-warning' : 'text-success';
