...
(function(app) {
    ...
    function initializeSalesChart() {
        ...
        salesChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['Coin', 'Hopper', 'Soap', 'Vending', 'Drop Off 1', 'Drop Off 2', 'Drop Off 3'],
                datasets: [{
                    label: 'Sales Amount ($)',
                    data: [],
                    backgroundColor: '#3b82f6',
    ...
    function updateSalesChart(data) {
        if (!salesChart) {
            salesChart = initializeSalesChart();
        }
        ...
        data.forEach(row => {
            totals.coin += parseFloat(row.coin) || 0;
            totals.hopper += parseFloat(row.hopper) || 0;
            totals.soap += parseFloat(row.soap) || 0;
            totals.vending += parseFloat(row.vending) || 0;
            totals.drop_off_1 += parseFloat(row.drop_off_1) || 0;
            totals.drop_off_2 += parseFloat(row.drop_off_2) || 0;
            totals.drop_off_3 += parseFloat(row.drop_off_3) || 0;
        });
    ...
    function updateSalesTable(data) {
        ...
        data.forEach(row => {
            ...
            tr.innerHTML = `
                ...
                <td class="px-4 py-2">$${row.coin.toFixed(2)}</td>
                <td class="px-4 py-2">$${row.hopper.toFixed(2)}</td>
                <td class="px-4 py-2">$${row.soap.toFixed(2)}</td>
                <td class="px-4 py-2">$${row.vending.toFixed(2)}</td>
                <td class="px-4 py-2">$${row.drop_off_1.toFixed(2)}</td>
                <td class="px-4 py-2">$${row.drop_off_2.toFixed(2)}</td>
                <td class="px-4 py-2">$${row.drop_off_3.toFixed(2)}</td>
            `;
            tbody.appendChild(tr);

            total += row.coin + row.hopper + row.soap + row.vending + 
                    row.drop_off_1 + row.drop_off_2 + row.drop_off_3;
        });

        document.getElementById('totalSales').textContent = `Total: $${total.toFixed(2)}`;
    }
})(window.LaundryAdmin); 