  ...
  try {
    ...
    if (deletedInventoryIds && deletedInventoryIds.length > 0) {
      try {
        ...
        // Convert IDs to MongoDB ObjectIds and handle any local- prefixes
        const objectIds = deletedInventoryIds.map(id => {
          console.log('\nProcessing ID:', id);
          ...
          try {
            ...
            console.error('Failed to convert to ObjectId:', cleanId, error);
            return null;
          }
        }).filter(id => id !== null);

        console.log('\nConverted ObjectIds:', objectIds);
        
        // First check if items exist
        const existingItems = await Inventory.find({ _id: { $in: objectIds } });
        console.log('\nFound existing items:', existingItems);
        ...
        if (existingItems.length === 0) {
          ...
          if (deleteResult.deletedCount > 0) {
            ...
            deletedItems.push(...existingItems);
          } else {
            console.log('No items were deleted');
            errors.push({ 
              type: 'inventory_delete', 
              error: 'Delete operation completed but no items were deleted' 
            });
          }
