...
router.get('/', async (req, res) => {
  try {
    ...
    const query = status === 'all' ? {} : { status: status || 'active' }; // Return all employees if status=all
    
    const employees = await Employee.find(query)
      .select('-__v')
      .sort({ name: 1 });
    
    console.log(`📋 Fetched ${employees.length} employees${status === 'all' ? '' : ` with status: ${status || 'active'}`}`);
    res.json(employees);
  } catch (err) {
