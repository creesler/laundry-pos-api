        ...
        for (const itemId of deletedInventoryIds) {
          try {
            ...
            if (deletedItem) {
              ...
              errors.push({ type: 'inventory_delete', error: `Item not found: ${itemId}` });
            }
          } catch (deleteError) {
            console.error('Error deleting individual item:', itemId, deleteError);
            errors.push({ type: 'inventory_delete', error: `Failed to delete item ${itemId}: ${deleteError.message}` });
          }
        }
      } catch (error) {
