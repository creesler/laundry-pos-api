...
router.get('/', async (req, res) => {
  try {
    ...
    const [sales, timesheets, inventory, inventoryLogs] = await Promise.all([
      Sale.find().sort({ date: -1 }),
      Timesheet.find().sort({ date: -1 }),
      Inventory.find().sort({ name: 1 }),
      InventoryLog.find().sort({ timestamp: -1 })
    ]);
