...
router.get('/', async (req, res) => {
  try {
    ...
    // First get all sales to check the data
    const allSales = await Sale.find({}).lean();
    console.log('All sales in DB:', JSON.stringify(allSales, null, 2));
    ...
    if (startDate && endDate) {
      ...
      console.log('Filtering sales between:', {
        startDate: start.toISOString(),
        endDate: end.toISOString()
      });

      // Filter sales where Date field matches the date range
      const filteredSales = allSales.filter(sale => {
        const saleDate = new Date(sale.Date);
        console.log('Comparing sale date:', {
          saleDate: saleDate.toISOString(),
          isWithinRange: saleDate >= start && saleDate <= end
        });
        return saleDate >= start && saleDate <= end;
      });

      console.log('Filtered sales:', JSON.stringify(filteredSales, null, 2));
      return res.json(filteredSales);
    }

    // If no date range provided, return all sales
    res.json(allSales);
  } catch (err) {
    ...
    const pipeline = [
      ...
      },
      {
        $group: {
          ...
          dropOffAmount1Total: {
            $sum: { $toDouble: "$Drop Off Amount 1" }
          },
          dropOffAmount2Total: {
            $sum: { $toDouble: "$Drop Off Amount 2" }
          }
        }
      },
      {
