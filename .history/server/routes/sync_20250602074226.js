...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (deletedInventoryIds && deletedInventoryIds.length > 0) {
      try {
        console.log('Processing deletions for IDs:', deletedInventoryIds);
        
        // Get all existing items that match the IDs
        const existingItems = await Inventory.find({
          _id: { $in: deletedInventoryIds.map(id => {
            console.log('Processing ID:', id);
            // Handle both MongoDB ObjectIds and local IDs
            return id.startsWith('local-') ? id.replace('local-', '') : id;
          })}
        });
        
        console.log('Found existing items for deletion:', existingItems);
        ...
        for (const item of existingItems) {
          try {
            ...
            console.log('Successfully deleted inventory item:', item._id);
          } catch (deleteError) {
            console.error('Error deleting item:', item._id, deleteError);
            errors.push({ type: 'inventory_delete', error: `Failed to delete ${item._id}: ${deleteError.message}` });
          }
        }

        // Double check if items were actually deleted
        const remainingItems = await Inventory.find({
          _id: { $in: existingItems.map(item => item._id) }
        });
