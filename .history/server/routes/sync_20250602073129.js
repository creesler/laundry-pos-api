...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (deletedInventoryIds && deletedInventoryIds.length > 0) {
      try {
        for (const itemId of deletedInventoryIds) {
          try {
            // Find the item in inventory first to get its name
            const itemToDelete = inventory.find(item => item.id === itemId);
            if (!itemToDelete) {
              console.log('Item not found in inventory data:', itemId);
              continue;
            }

            // Delete by name since it's unique
            const deletedItem = await Inventory.findOneAndDelete({ name: itemToDelete.name });
            if (deletedItem) {
              deletedItems.push(deletedItem);
              console.log('Successfully deleted inventory item:', itemToDelete.name);
            } else {
              console.log('Item not found in database:', itemToDelete.name);
              errors.push({ type: 'inventory_delete', error: `Item not found: ${itemToDelete.name}` });
            }
          } catch (deleteError) {
            console.error('Error deleting individual item:', deleteError);
            errors.push({ type: 'inventory_delete', error: deleteError.message });
          }
        }
      } catch (error) {
    ...
    if (inventory && inventory.length > 0) {
      try {
        for (const item of inventory) {
          ...
          try {
            // Use _id for updates, fallback to creating new if not found
            const query = item._id ? { _id: item._id } : { name: item.name };
