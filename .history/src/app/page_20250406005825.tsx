...
export default function Home() {
  ...
  // Update calculateDailyTotals to use start and end date
  const calculateDailyTotals = () => {
    const dailyTotals = new Map()
    
    // Use date range for calculations
    ...
    while (currentDate <= end) {
      ...
      if (!dailyTotals.has(datePart)) {
        dailyTotals.set(datePart, 0.001) // Set very small value for zero to show at bottom
      }
      currentDate.setDate(currentDate.getDate() + 1)
    }
    ...
    const sortedEntries = Array.from(dailyTotals.entries())
      ...
      .map(([date, total]) => {
        ...
        const dateFormat = startMonth !== endMonth ? 
          `${dateObj.getMonth() + 1}/${dateObj.getDate()}` : // Show month/day if spanning multiple months
          dateObj.getDate().toString() // Show only day if within same month
