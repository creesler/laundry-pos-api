...
export default function Home() {
  ...
  const syncEmployeeList = async () => {
    try {
      ...
      // Fetch all employees from MongoDB (both active and inactive)
      const response = await fetch(`${API_URL}/employees?status=all`);
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Failed to fetch employee data from server');
      }
      ...
      throw new Error('Failed to sync employee list: ' + (error instanceof Error ? error.message : 'Unknown error'));
    }
  };
