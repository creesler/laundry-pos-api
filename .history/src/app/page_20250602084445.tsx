...
export default function Home() {
  ...
  const handleDeleteInventoryItem = async (itemId: string) => {
    try {
      ...
      // If it's a MongoDB ID (not a local ID), add it directly to deletedInventoryIds
      const idToDelete = itemId.startsWith('local-') ? itemId : itemToDelete._id;
      ...
      console.log('Adding to deletedInventoryIds:', idToDelete);
      setDeletedInventoryIds(prev => {
        const newDeletedIds = [...prev, idToDelete];
      ...
      const updatedData = {
        ...
        deletedInventoryIds: [...(existingData.deletedInventoryIds || []), idToDelete]
      };
  ...
  const saveToServer = async () => {
    try {
      ...
      const serverIdsToDelete = deletedInventoryIds
        .map(id => {
          // If it's already a MongoDB ID, use it directly
          if (!id.startsWith('local-')) {
            return id;
          }
          // For local IDs, find the corresponding server item by name
          const localItem = inventoryItems.find(item => item.id === id);
      ...
      // If sync was successful, update local state
      const updatedTimeData = employeeTimeData.map(entry => 
