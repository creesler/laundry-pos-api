...
export default function Home() {
  ...
  const saveToServer = async () => {
    try {
      console.log('Starting saveToServer function...');
      
      // Check if online first
      if (!isOnline) {
        console.log('Device is offline, cannot save');
        setSnackbar({
          open: true,
      ...
      console.log('Unsaved entries found:', {
      ...
      if (unsavedTimesheetEntries.length === 0 && unsavedSalesEntries.length === 0) {
        console.log('No unsaved entries found, proceeding with employee sync anyway');
      }
      ...
      try {
        ...
        console.log('Fetching from:', 'http://localhost:5000/api/employees');
        ...
        if (!employeeResponse.ok) {
          ...
          throw new Error(`Failed to fetch employee data: ${employeeResponse.status} ${employeeResponse.statusText}`);
        }
        
        const serverEmployees: Employee[] = await employeeResponse.json();
        console.log('Server employees fetched:', serverEmployees);
        ...
        console.log('Server active employees:', serverEmployeeNames);
        console.log('Local employees:', localEmployeeList);

        // Find changes
        const newEmployees = serverEmployeeNames.filter((emp: string) => !localEmployeeList.includes(emp));
        const deletedEmployees = localEmployeeList.filter((emp: string) => !serverEmployeeNames.includes(emp));
        
        // Build sync message
        const changes: string[] = [];
        if (newEmployees.length > 0) {
          console.log('New employees found:', newEmployees);
          changes.push(`Added ${newEmployees.length} new employee(s)`);
        }
        if (deletedEmployees.length > 0) {
          console.log('Removed employees:', deletedEmployees);
          changes.push(`Removed ${deletedEmployees.length} employee(s)`);
        }
        ...
        if (selectedEmployee && !serverEmployeeNames.includes(selectedEmployee)) {
          ...
          if (deletedEmployees.includes(selectedEmployee)) {
            const updatedTimeData = employeeTimeData.map(entry => 
            ...
            await saveToIndexedDB({
              ...indexedDBData,
              employeeTimeData: updatedTimeData,
              lastEmployeeSync: new Date().toISOString()
            });

            changes.push(`Updated entries for removed employee: ${selectedEmployee}`);
          }
        }
        ...
        // Set sync message
        syncMessage = changes.length > 0 
          ? `Employee sync complete: ${changes.join(', ')}`
          : 'Employee list is up to date';

        console.log('Employee sync completed successfully');

      } catch (error) {
        console.error('Error syncing employees:', error);
        throw new Error('Failed to sync employees: ' + (error instanceof Error ? error.message : 'Unknown error'));
      }
      ...
      try {
        ...
        const finalMessage = [syncMessage];
        if (unsavedTimesheetEntries.length > 0) finalMessage.push(`Saved ${unsavedTimesheetEntries.length} timesheet entries`);
        if (unsavedSalesEntries.length > 0) finalMessage.push(`Saved ${unsavedSalesEntries.length} sales entries`);
