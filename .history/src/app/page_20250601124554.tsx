  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        ...
        if (localData) {
          // Always prioritize local data for sales and timesheet
        ...
        // Only fetch employee list from server if online
        if (isOnline) {
          try {
            const response = await fetch('http://localhost:5000/api/employees');
            if (response.ok) {
              ...
              // Save employee list to IndexedDB while preserving other data
              const existingData = await getFromIndexedDB() || {};
              await saveToIndexedDB({
                ...existingData,
                employeeList: employees,
            ...
            console.error('Error fetching employee list:', error);
            // If server fetch fails, use cached employee list
            if (localData?.employeeList) {
          ...
          // If offline, use cached employee list
          if (localData?.employeeList) {
            setEmployeeList(localData.employeeList);
            const lastSelectedEmployee = localStorage.getItem('selectedEmployee');
            if (lastSelectedEmployee && localData.employeeList.includes(lastSelectedEmployee)) {
