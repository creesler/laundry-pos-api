...
export default function Home() {
  ...
  const handleUpdateInventory = async (
    ...
    try {
      // Find the current item to get its previous stock
      const currentItem = inventoryItems.find(item => item.id === itemId);
      if (!currentItem) {
        throw new Error('Item not found');
      }

      // For initial stock (when maxStock is 0), set maxStock to match the stock level
      const newMaxStock = currentItem.maxStock === 0 ? newStock : currentItem.maxStock;
      ...
      // Update state immediately with a new array reference
