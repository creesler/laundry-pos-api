...
export default function Home() {
  ...
  const handleUpdateInventory = useCallback((itemId: string, newStock: number) => {
    ...
    setInventoryItems((prev: InventoryItem[]) => prev.map(item => 
      item.id === itemId 
    ...
    setInventoryLogs(prev => [...prev, {
      itemId,
      oldStock: currentItem.currentStock,
      newStock,
  ...
  const handleDeleteInventoryItem = useCallback((itemName: string) => {
    setInventoryItems(prev => prev.filter(item => item.name !== itemName));
  }, []);
  ...
  const handleSaveToServer = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to save inventory changes');
      }

      setInventoryLogs(prev => prev.map(log => ({ ...log, isSaved: true })));
    } catch (error) {
