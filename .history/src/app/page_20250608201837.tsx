...
// Update the Employee interface at the top of the file
interface Employee {
  _id: string;
  name: string;
  status: string;
...
export default function Home() {
  ...
  const saveToServer = async () => {
    try {
      ...
      console.log('🔄 Updating employee dropdown list:');
      console.log('- Current local list:', localEmployeeList);
      console.log('- New server list:', serverEmployeeNames);
      ...
      if (deletedEmployees.length > 0) {
        console.log('🗑️ Removed employees:', deletedEmployees);
      }

      // Update both state and local storage with active employees
      console.log('🔄 Updating employee list state...');
      
      // Update the employee list state
      const activeEmployeeNames = serverEmployees
        .filter(emp => emp.status === 'active')
        .map(emp => emp.name);
      setEmployeeList(activeEmployeeNames);
      
      // Update localStorage
      localStorage.setItem('employeeList', JSON.stringify(activeEmployeeNames));
      ...
      console.log('💾 Employee list saved to IndexedDB');
