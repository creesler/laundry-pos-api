...
export default function Home() {
  ...
  const fetchTimesheetData = async () => {
    ...
    try {
      const response = await fetch(
        `/api/timesheets?employeeName=${encodeURIComponent(selectedEmployee)}&startDate=${timesheetDateRange.startDate.toISOString().split('T')[0]}&endDate=${timesheetDateRange.endDate.toISOString().split('T')[0]}`
      );
      
      if (!response.ok) throw new Error('Failed to fetch timesheet data');
      
      const data = await response.json() as TimesheetAPIResponse[];
      setTimesheetData(data.map((entry) => ({
        ...entry,
        status: entry.clockOut ? 'Completed' : 'Pending',
        timeIn: formatDateTime(entry.clockIn, 'time'),
        timeOut: entry.clockOut ? formatDateTime(entry.clockOut, 'time') : '--',
        date: formatDateTime(entry.date, 'date'),
  ...
  const formatDateTime = (dateString: string, format: 'date' | 'time' = 'date') => {
  ...
  // Update saveToServer function to first sync employees from server, then upload local changes
  const saveToServer = async () => {
    try {
      ...
      // STEP 1: Download and sync employee data from server
      setSnackbar({
        open: true,
        message: 'Checking for employee updates from server...',
        severity: 'info'
      });
      ...
      await saveToIndexedDB({
        ...
        console.log('Successfully synced employee list from server');
    } catch (error) {
      ...
      // STEP 2: Upload any unsaved local changes to server
      ...
      if (unsavedTimesheetEntries.length === 0 && unsavedSalesEntries.length === 0 && unsavedInventoryLogs.length === 0) {
        setSnackbar({
          open: true,
          message: 'Employee list updated. No new entries to upload.',
          severity: 'info'
        });
        return;
      }
      ...
      setSnackbar({
        open: true,
        message: 'Uploading new entries to server...',
        severity: 'info'
      });

      // Upload all unsaved data to server
      const uploadResponse = await fetch('http://localhost:5000/api/sync', {
        method: 'POST',
        headers: {
      ...
      // Mark all uploaded entries as saved
      const updatedTimeData = employeeTimeData.map(entry => 
        !entry.isSaved ? { ...entry, isSaved: true } : entry
      );
      ...
      setSnackbar({
        open: true,
        message: 'Successfully uploaded all new entries to server',
        severity: 'success'
      });
