...
export default function Home() {
  ...
  const saveToServer = async () => {
    try {
      ...
      setEmployeeList([]); // Clear the list first
      setTimeout(() => {
        setEmployeeList(serverEmployeeNames); // Then update with new data
      }, 0);
      
      // Check if currently selected employee is still valid
      if (selectedEmployee && !serverEmployeeNames.includes(selectedEmployee)) {
        // If not valid, select the first available employee or clear selection
      ...
      await saveToIndexedDB({
        ...
        lastSelectedEmployee: selectedEmployee // Save last selected employee
      });
