...
export default function Home() {
  ...
  const saveToServer = async () => {
    try {
        ...
        const salesResponse = await fetch('http://localhost:5000/api/sales/bulk', {
          ...
          body: JSON.stringify({
            entries: unsavedSalesEntries.map(entry => ({
              date: entry.Date.split('|')[0], // Remove the sortable timestamp part
      ...
      // Update local state to mark all entries as saved
      const updatedTimeData = employeeTimeData.map(entry => 
      ...
      // Only fetch updated employee list
  ...
  // Add initialization effect at the top of the component
  useEffect(() => {
    const initializeData = async () => {
      try {
        const data = await getFromIndexedDB();
        if (data) {
          // Set saved data if it exists
          if (data.data && Array.isArray(data.data)) {
            setSavedData(data.data);
          }
          // Set employee time data if it exists
          if (data.employeeTimeData && Array.isArray(data.employeeTimeData)) {
            setEmployeeTimeData(data.employeeTimeData);
          }
          // Set employee list if it exists
          if (data.employeeList && Array.isArray(data.employeeList)) {
            setEmployeeList(data.employeeList);
            // Set selected employee if there was one previously selected
            const lastSelectedEmployee = localStorage.getItem('selectedEmployee');
            if (lastSelectedEmployee && data.employeeList.includes(lastSelectedEmployee)) {
              setSelectedEmployee(lastSelectedEmployee);
            } else if (data.employeeList.length > 0) {
              setSelectedEmployee(data.employeeList[0]);
            }
          }
        }
  ...
  return (
    <>
      ...
      <TimesheetModal
        ...
        timesheetData={timesheetData.map(record => ({
          ...record,
