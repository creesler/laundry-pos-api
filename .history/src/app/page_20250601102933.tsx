...
export default function Home() {
  ...
  const saveToServer = async () => {
    try {
      ...
      const formattedEntries = employeeEntries.map(entry => ({
        date: entry.date,
        duration: entry.duration || calculateDuration(entry.time, entry.time), // For single entries
      ...
      const response = await fetch('http://localhost:5000/api/timesheets/bulk', {
        ...
        body: JSON.stringify({
          ...
          totalHours: employeeEntries.length // This will be recalculated on server
        })
      });
  ...
  return (
    <>
      ...
      {/* Replace the SAVE button in the return JSX */}
      <Button
        variant="contained"
