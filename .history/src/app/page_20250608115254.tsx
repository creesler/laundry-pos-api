...
export default function Home() {
  ...
  // Add the saveToServer function
  const saveToServer = async () => {
    try {
      console.log('[SAVE TO SERVER] Starting employee sync...');
      ...
      const serverEmployees = await employeeResponse.json();
      console.log('[EMPLOYEE SYNC] Received employees from server:', serverEmployees);
      
      // Update local employee list with server data
      const serverEmployeeNames = serverEmployees
        .filter((emp: { status: string }) => emp.status === 'active')
        .map((emp: { name: string }) => emp.name);
      console.log('[EMPLOYEE SYNC] Filtered active employees:', serverEmployeeNames);
      
      // Force update of employee list state
      console.log('[EMPLOYEE SYNC] Current employee list:', employeeList);
      setEmployeeList([]);
      console.log('[EMPLOYEE SYNC] Cleared employee list');
      
      // Update with new data immediately
      setEmployeeList(serverEmployeeNames);
      console.log('[EMPLOYEE SYNC] Updated employee list with:', serverEmployeeNames);
      
      // Check if currently selected employee is still valid
      if (selectedEmployee && !serverEmployeeNames.includes(selectedEmployee)) {
        console.log('[EMPLOYEE SYNC] Current selected employee not found in new list, updating selection');
        setSelectedEmployee(serverEmployeeNames.length > 0 ? serverEmployeeNames[0] : '');
      }
      ...
      console.log('[EMPLOYEE SYNC] Updated IndexedDB with new employee list');

      // Step 2: Proceed with timesheet sync
      console.log('[TIMESHEET SYNC] Starting timesheet sync...');
      const unsavedEntries = employeeTimeData.filter(entry => !entry.isSaved);
      console.log('[TIMESHEET SYNC] Unsaved entries:', unsavedEntries);
      ...
      console.log('[TIMESHEET SYNC] Successfully saved timesheets to server');
      ...
      setEmployeeTimeData(updatedTimeData);
      console.log('[TIMESHEET SYNC] Updated local timesheet data');

      // Update IndexedDB with all changes
      await saveToIndexedDB({
        ...existingData,
        employeeTimeData: updatedTimeData,
        employeeList: serverEmployeeNames,
      ...
      console.log('[FINAL] All data synchronized successfully');
      ...
      console.error('[ERROR] Error in saveToServer:', error);
      setSnackbar({
        open: true,
