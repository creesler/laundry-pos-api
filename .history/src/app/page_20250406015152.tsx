...
import { saveToIndexedDB, getFromIndexedDB, saveCSVToIndexedDB, getCSVFromIndexedDB } from './utils/db'
...
export default function Home() {
    ...
    const loadData = async () => {
      try {
        ...
        if (indexedDBData) {
          ...
          setSavedData(indexedDBData)
          // Update localStorage for backwards compatibility
          localStorage.setItem('laundry_data', JSON.stringify(indexedDBData))
        } else {
          ...
          if (savedDataStr) {
            ...
            await saveToIndexedDB(dummyData)
          }
        }
  ...
  // Add function to update CSV in IndexedDB
  const updateCSV = async () => {
    try {
      const worksheet = XLSX.utils.json_to_sheet(savedData)
      const csvContent = XLSX.utils.sheet_to_csv(worksheet)
      await saveCSVToIndexedDB(csvContent)
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingIndex !== null) {
        ...
        const originalTime = originalDateTime.slice(1).join(' ')
        dateTimeString = `${inputValues.Date} ${originalTime}`
        setCurrentFormDate(inputValues.Date)
      } else {
        const now = new Date()
      ...
      // Update state and both storage systems
  ...
  // Update email sending function to use IndexedDB
  const sendEmailWithCSV = async () => {
    try {
      // Get CSV from IndexedDB
      const csvContent = await getCSVFromIndexedDB()
      ...
      const totalSales = calculateDailyTotals().reduce((sum, day) => sum + (day.total / 50), 0).toFixed(2)
      
      // Create email body with data
      const body = `Daily Sales Report\n\nTotal Sales: $${totalSales}\n\n` +
                  `--- Raw Data Below ---\n\n${csvContent}`
      
      // Open default mail app with the data in body
      window.location.href = `mailto:creesler@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`
      
      setSnackbar({
        open: true,
        message: 'Email opened with data included',
        severity: 'success'
      })
    } catch (error) {
          ...
          <Paper sx={{ 
            ...
            <Box sx={{ 
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                >
                  <ShareIcon sx={{ color: blue[600], fontSize: '24px' }} />
                </IconButton>
              </Box>
              <Typography fontSize="2vh" color="textSecondary" sx={{ ml: 2 }}>Laundry Shop POS Daily Entry</Typography>
              <Menu
              ...
              >
                ...
                >
                  <EmailIcon sx={{ color: blue[600] }} />
                  <Typography>Email to creesler@gmail.com</Typography>
                </MenuItem>
                <MenuItem onClick={handleBluetoothShare} sx={{ gap: '1vh', py: 1.5 }}>
                  <BluetoothIcon sx={{ color: blue[600] }} />
                  <Typography>Share via Bluetooth</Typography>
                </MenuItem>
              </Menu>
            </Box>
