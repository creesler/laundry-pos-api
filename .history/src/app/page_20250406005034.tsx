...
export default function Home() {
  ...
  const calculateDailyTotals = () => {
    ...
    if (timePeriod === 'daily') {
      // Show last 30 days
      startDate.setDate(endDate.getDate() - 29) // Start from 30 days ago
    } else if (timePeriod === 'week') {
      // Calculate week start and end dates
      const today = new Date()
      const currentWeek = Math.ceil(today.getDate() / 7)
      const selectedWeekDiff = currentWeek - selectedWeek
      
      // Set start date to beginning of selected week
      startDate = new Date()
      startDate.setDate(startDate.getDate() - (startDate.getDay() - 1) - (selectedWeekDiff * 7))
      
      // Set end date to end of selected week
      endDate.setTime(startDate.getTime())
      endDate.setDate(startDate.getDate() + 6)
    } else if (timePeriod === 'month') {
      // Set to selected month
      startDate = new Date(endDate.getFullYear(), selectedMonth, 1)
      endDate.setTime(startDate.getTime())
    ...
    if (timePeriod === 'year') {
      ...
      for (let month = 0; month < 12; month++) {
        const monthKey = `${month + 1}/${selectedYear}`
        dailyTotals.set(monthKey, 0)
      }
    } else if (timePeriod === 'month') {
      ...
      // Initialize week days with zero
      for (let i = 0; i < 7; i++) {
    ...
    const sortedEntries = Array.from(dailyTotals.entries())
      .sort((a, b) => {
        if (timePeriod === 'year') {
          // Sort by month/year for year view
          ...
          // Sort by date for week and month views
          return new Date(a[0]).getTime() - new Date(b[0]).getTime()
        }
      })
      .map(([date, total]) => {
        if (timePeriod === 'year') {
          // For year view, keep month/year format
          const [month, year] = date.split('/')
          const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
          return {
            date: monthNames[parseInt(month) - 1],
            total: total === 0 ? 0.001 : parseFloat(total.toFixed(2)) // Set very small value for zero to show at bottom
          }
        } else {
          // For week and month views, show day only
              ...
              <Box sx={{ display: 'flex', gap: '1vh', alignItems: 'center' }}>
                ...
                <ButtonGroup size="small" sx={{ height: '3vh' }}>
                  ...
                  <Button 
                    ...
                    sx={{ fontSize: '1.4vh', py: 0 }}
                  >
                    {new Date(2024, selectedMonth).toLocaleString('default', { month: 'short' })}
                  </Button>
                  <Menu
