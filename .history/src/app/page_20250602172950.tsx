...
export default function Home() {
  ...
  // Load timesheet data from IndexedDB when modal opens or employee changes
  useEffect(() => {
    if (selectedEmployee && timesheetOpen) {
      loadLocalTimesheetData();
    }
  }, [selectedEmployee, timesheetOpen]);

  // Function to load timesheet data from IndexedDB
  const loadLocalTimesheetData = async () => {
    try {
  ...
  // Update fetchTimesheetData to be used only by the Request Data button
  const fetchTimesheetData = async () => {
    ...
    try {
      ...
      // Don't fall back to local data on API error
      setSnackbar({
        open: true,
