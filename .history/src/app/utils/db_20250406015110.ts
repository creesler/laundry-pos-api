...
// Save CSV to IndexedDB
export const saveCSVToIndexedDB = async (csvContent: string) => {
  try {
    const db = await openDB()
    const transaction = db.transaction(STORE_NAME, 'readwrite')
    const store = transaction.objectStore(STORE_NAME)
...
export const getCSVFromIndexedDB = async (): Promise<string | null> => {
  try {
    const db = await openDB()
    const transaction = db.transaction(STORE_NAME, 'readonly')
    const store = transaction.objectStore(STORE_NAME)
    ...
    console.error('Error reading CSV from IndexedDB:', error)
    return null
  }
} 