...
export default function Home() {
  ...
  const saveToServer = async () => {
    try {
      ...
      const formattedEntries: FormattedTimeEntry[] = [];
      Object.entries(entriesByDate).forEach(([date, entries]) => {
      ...
      const response = await fetch('http://localhost:5000/api/timesheets/bulk', {
        ...
        body: JSON.stringify({
          ...
          totalHours: formattedEntries.reduce((total, entry) => {
      ...
      // After saving to server, also save to Google Sheets
  ...
  return (
    <>
      <Box sx={{ 
        ...
        <Box sx={{
          ...
          <Box sx={{ 
            ...
            gridTemplateColumns: {
              xs: '1fr',
              md: '3fr 2fr'  // Adjusted ratio for better space utilization
            },
            gridTemplateRows: {
              xs: 'auto auto auto auto auto',
              md: 'auto 2fr 1fr auto'  // Adjusted row sizes
            },
            gap: 2,  // Increased gap for better spacing
            maxWidth: '100%',
            maxHeight: '100%',
            overflow: 'hidden',
            p: 2  // Added padding around the grid
          }}>
            <Header
              ...
              sx={{ gridArea: 'header', mb: 1 }}  // Added margin bottom
            />
      ...
      {/* Move SAVE button inside the form area */}
      <Box sx={{ 
        position: 'fixed',
        bottom: 0,
        left: 0,
