...
export default function Home() {
  ...
  const syncEmployeeList = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: 'Error syncing employee list with server',
        severity: 'error'
      });
      return false;
    }
  ...
  // Add useEffect to sync on mount and when online status changes
  useEffect(() => {
    if (isOnline) {
      syncEmployeeList();
    }
  ...
  // Modify saveToServer function to use syncEmployeeList
  const saveToServer = async () => {
    try {
      console.log('🔄 TEST: saveToServer function called');
      ...
      // Rest of the existing saveToServer function...
