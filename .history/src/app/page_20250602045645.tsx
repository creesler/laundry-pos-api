...
export default function Home() {
    ...
    const loadData = async () => {
      try {
        ...
        if (indexedDBData?.data) {
          ...
          const filteredData = indexedDBData.data.map((item: any) => ({
            ...item,
  ...
  const handleNumpadClick = (value: string) => {
    ...
    setInputValues(prev => {
      ...
      if (value === 'Del') {
        ...
        if (value.match(/[0-9]/) || (value === '.' && !currentValue.includes('.'))) {
          newValue = currentValue + value
        }
      }

      // Return same object reference if value hasn't changed
  ...
  // Optimize handleSave to reduce unnecessary operations
  const handleSave = async () => {
    try {
      ...
      const dateTimeString = editingIndex !== null
        ? `${inputValues.Date} ${displayTime}`
        : `${currentDate} ${displayTime}`;

      // Only create new record if there's actual data
      const hasData = Object.entries(inputValues).some(([key, value]) => 
      ...
      const updatedData = editingIndex !== null
        ? savedData.map((item, index) => index === editingIndex ? newRecord : item)
        : [newRecord, ...savedData];
      ...
      try {
        ...
        await saveToIndexedDB({
          ...
          employeeList: existingData.employeeList || [],
          inventory: existingData.inventory || [],
          inventoryLogs: existingData.inventoryLogs || []
        });
      } catch (error) {
        console.error('Background save to IndexedDB failed:', error);
      ...
      const updatedItems = inventoryItems.map(item => {
        if (item.id === itemId) {
          if (updateType === 'usage') {
            ...
            // For restock, set new maxStock but preserve currentStock
            return {
              ...item,
              maxStock: Number(newStock),
              currentStock: item.currentStock, // Preserve current stock
              lastUpdated: new Date().toISOString()
            };
          }
        }
