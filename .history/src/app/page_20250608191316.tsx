...
export default function Home() {
  ...
  const saveToServer = async () => {
    try {
      console.log('🔄 Starting employee synchronization with MongoDB...');
      
      // Step 1: Fetch and sync employee data from MongoDB
      console.log('📡 Fetching employee data from MongoDB...');
      ...
      // Log changes in employee list
      const newEmployees = serverEmployeeNames.filter(emp => !currentList.includes(emp));
      ...
      const response = await fetch('http://localhost:5000/api/timesheets/bulk', {
        ...
        body: JSON.stringify({
          ...
          entries: employeeTimeData.filter(entry => !entry.isSaved)
        })
      });
      ...
      console.log('✅ Timesheet data saved to server');
      ...
      // Update IndexedDB with saved timesheet data
      console.log('📦 Updating IndexedDB with timesheet data...');
      await saveToIndexedDB({
        ...existingData,
        employeeTimeData: updatedTimeData,
        employeeList: serverEmployeeNames
      ...
      // Force a re-render of components that depend on employee list
      setEmployeeList([...serverEmployeeNames]);
