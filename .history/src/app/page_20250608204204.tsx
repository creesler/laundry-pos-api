...
export default function Home() {
  ...
  // Handle employee list changes
  useEffect(() => {
    if (employeeList.length > 0) {
      // Update IndexedDB
      const updateIndexedDB = async () => {
        try {
          console.log('🔄 Updating employee list in IndexedDB:', employeeList);
          const indexedDBData = await getFromIndexedDB() || {};
          await saveToIndexedDB({
            ...indexedDBData,
            employeeList
          });
          console.log('✅ Employee list updated in IndexedDB');
        } catch (error) {
          console.error('❌ Error updating employee list in IndexedDB:', error);
        }
      };
      updateIndexedDB();
      ...
      // If no employee is selected, select the first one
      if (!selectedEmployee && employeeList.length > 0) {
        const firstEmployee = employeeList[0];
        console.log('👤 No employee selected, selecting first available:', firstEmployee);
