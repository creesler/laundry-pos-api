...
export default function Header({ 
  ...
  const handleInventoryUpdate = (item: { id: string; itemId: string; previousStock: number; newStock: number; updateType: 'restock' | 'usage' | 'adjustment'; timestamp: string; updatedBy: string; notes?: string; isSaved: boolean }) => {
  ...
  const handleSaveTimeEntry = (entry: TimeEntry) => {
    setEmployeeTimeData(prev => [...prev, {
      ...entry,
      clockOutTime: entry.action === 'out' ? entry.time : undefined
    }])
  }

  const handleSaveInventoryUpdate = (update: InventoryUpdate) => {
