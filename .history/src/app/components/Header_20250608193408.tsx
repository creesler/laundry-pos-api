...
export default function Header({ 
  ...
  const { employees, selectedEmployee, setSelectedEmployee, isLoading, error, refreshEmployees } = useEmployees()
    ...
    const loadInitialState = async () => {
      try {
        ...
        if (data?.employeeTimeData) {
          ...
          const employeeEntries = data.employeeTimeData.filter(entry => entry.employeeName === selectedEmployee)
  ...
  }, [selectedEmployee, employeeTimeData])
  ...
  const handleEmployeeChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    const newEmployee = event.target.value as string
  ...
  const saveEmployeeTimeLocally = async (action: 'in' | 'out') => {
    ...
    try {
      ...
      const pendingResponse = await fetch(`http://localhost:5000/api/timesheets?employeeName=${selectedEmployee}&status=pending`);
      ...
      if (action === 'in') {
        ...
        setSnackbar({
          open: true,
          message: `${selectedEmployee} successfully clocked in!`,
          severity: 'success'
        })
      } else {
        ...
        setSnackbar({
          open: true,
          message: `${selectedEmployee} successfully clocked out!`,
          severity: 'success'
        })
    ...
    try {
      ...
      const updateLogs = itemsToUpdate.map(({ itemId, usage }) => {
        ...
        return {
          ...
          notes: `Used ${usage} ${item.unit} by ${selectedEmployee}`,
          isSaved: false
        };
      }).filter(Boolean);
        ...
        <Box display="flex" alignItems="center" gap="1vh" justifyContent={{ xs: 'space-between', md: 'flex-end' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: '1vh' }}>
            ...
            <Typography fontSize="2.8vh" fontWeight="bold" color={grey[600]} sx={{ ml: 2, mr: 1 }}>
              Hi{selectedEmployee ? ` ${selectedEmployee}` : ''}
            </Typography>
