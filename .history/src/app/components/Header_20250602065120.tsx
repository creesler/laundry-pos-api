    ...
    <>
      ...
      >
        ...
        <DialogContent>
          ...
          {/* Get soap item and calculate available stock */}
          {(() => {
            const soapItem = inventory.find(item => item.name === 'Laundry Soap');
            const soapAvailable = soapItem ? soapItem.maxStock - soapItem.currentStock : 0;
            return (
              <TextField
                autoFocus
                margin="dense"
                label={`Soap Used (${soapAvailable} ${soapItem?.unit || 'bottles'} available)`}
                type="number"
                fullWidth
                value={soapUsage}
                onChange={(e) => setSoapUsage(e.target.value)}
                disabled={!soapItem || soapAvailable <= 0}
                helperText={!soapItem ? "Item not found" : soapAvailable <= 0 ? "No stock available" : ""}
                InputProps={{
                  inputProps: { min: 0, max: soapAvailable }
                }}
              />
            );
          })()}
          {/* Get detergent item and calculate available stock */}
          {(() => {
            const detergentItem = inventory.find(item => item.name === 'Detergent');
            const detergentAvailable = detergentItem ? detergentItem.maxStock - detergentItem.currentStock : 0;
            return (
              <TextField
                margin="dense"
                label={`Detergent Used (${detergentAvailable} ${detergentItem?.unit || 'boxes'} available)`}
                type="number"
                fullWidth
                value={detergentUsage}
                onChange={(e) => setDetergentUsage(e.target.value)}
                disabled={!detergentItem || detergentAvailable <= 0}
                helperText={!detergentItem ? "Item not found" : detergentAvailable <= 0 ? "No stock available" : ""}
                InputProps={{
                  inputProps: { min: 0, max: detergentAvailable }
                }}
              />
            );
          })()}
