  ...
  const handleUpdateSubmit = () => {
    if (selectedItem && updateAmount) {
      ...
      if (currentItem) {
        ...
        // In 'add' mode, we're setting the total stock capacity
        // The used amount (currentStock) stays at 0
        const newMaxStock = amount;
        
        onUpdateStock(
          selectedItem, 
          0, // Keep used amount at 0
          'restock',
          `Set total stock capacity to ${amount} ${currentItem.unit}. ${updateNotes}`
        );
  ...
  return (
    <Paper sx={{ ...sx, p: '1.5vh', display: 'flex', flexDirection: 'column', borderRadius: '8px', border: '1px solid #e5e7eb', minHeight: 0, overflow: 'hidden', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
      ...
      <Box sx={{ 
        ...
        {inventory.map((item) => {
          const usagePercentage = (item.currentStock / item.maxStock) * 100;
