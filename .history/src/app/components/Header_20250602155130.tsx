...
export default function Header({ 
  ...
  const saveEmployeeTimeLocally = async (action: 'in' | 'out') => {
    ...
    try {
      ...
      const employee = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/employees?name=${encodeURIComponent(activeEmployee)}`)
        .then(res => res.json())
        .then(data => data[0])
      ...
      // Save to server
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/timesheets/${action === 'in' ? 'clock-in' : 'clock-out'}`, {
