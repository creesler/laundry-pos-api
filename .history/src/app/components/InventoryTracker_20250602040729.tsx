...
export default function InventoryTracker({ inventory, onUpdateStock, onAddItem, sx }: InventoryTrackerProps) {
  ...
  const handleUpdateClick = () => {
    handleClose();
    setUpdateType('restock');  // Force restock type
  ...
  const handleUpdateSubmit = () => {
    if (selectedItem && updateAmount) {
      ...
      if (currentItem) {
        ...
        const newStock = Math.min(currentItem.maxStock, currentItem.currentStock + amount);
        onUpdateStock(selectedItem, newStock, 'restock', updateNotes);
      }
    }
