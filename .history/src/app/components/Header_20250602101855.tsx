...
interface HeaderProps {
  ...
  onUpdateInventory: (itemId: string, newStock: number, updateType: 'sale', notes?: string) => void
  inventory: Array<{
    id: string
    name: string
    currentStock: number
...
export default function Header({ 
  ...
  const handleUsageSubmit = async () => {
    try {
      // Process all items with usage entered
      for (const [itemId, usage] of Object.entries(itemUsages)) {
        if (usage) {
          const item = inventory.find(i => i.id === itemId);
          if (item) {
            const usageAmount = Number(usage);
            if (usageAmount > 0) {
              await onUpdateInventory(
                itemId,
                usageAmount,
                'sale',
                `Used ${usageAmount} ${item.unit} by ${activeEmployee}`
              );
            }
          }
        }
