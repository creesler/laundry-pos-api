  ...
  const handleUsageSubmit = async () => {
    try {
      ...
      if (soapItem && soapUsage) {
        ...
        if (usage > 0) {
          const newStock = Math.max(0, soapItem.currentStock - usage)
          await onUpdateInventory(soapItem.id, newStock, 'usage', `Daily usage recorded by ${activeEmployee}`)
        }
      }
      ...
      if (detergentItem && detergentUsage) {
        ...
        if (usage > 0) {
          const newStock = Math.max(0, detergentItem.currentStock - usage)
          await onUpdateInventory(detergentItem.id, newStock, 'usage', `Daily usage recorded by ${activeEmployee}`)
        }
      }
