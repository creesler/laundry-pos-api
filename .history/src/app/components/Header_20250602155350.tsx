...
export default function Header({ 
  ...
  const saveEmployeeTimeLocally = async (action: 'in' | 'out') => {
    ...
    try {
      ...
      const employee = await fetch(`http://localhost:5000/api/employees?name=${encodeURIComponent(activeEmployee)}`)
        .then(res => {
          if (!res.ok) throw new Error('Failed to find employee');
          return res.json();
        })
      ...
      const newEntry = {
        ...
        clockIn: action === 'in' ? now.toISOString() : null,
        clockOut: action === 'out' ? now.toISOString() : null,
      ...
      // Save to server
      const response = await fetch(`http://localhost:5000/api/timesheets/${action === 'in' ? 'clock-in' : 'clock-out'}`, {
        method: action === 'in' ? 'POST' : 'PUT',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to clock ${action}: ${errorData}`);
      }
      ...
      if (action === 'in') {
        setClockedIn(true)
        setClockInTime(now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }))
        setClockOutTime('--')
        setSnackbar({
          open: true,
        ...
        setClockedIn(false)
        setClockOutTime(now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }))
        setTimeout(() => {
          setClockInTime('--')
          setClockOutTime('--')
