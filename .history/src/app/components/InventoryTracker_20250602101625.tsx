...
interface InventoryTrackerProps {
  ...
  onUpdateStock: (itemId: string, newStock: number, updateType: 'restock' | 'sale' | 'adjustment' | 'damage' | 'other', notes?: string) => void;
...
export default function InventoryTracker({ inventory, onUpdateStock, onAddItem, sx }: InventoryTrackerProps) {
  ...
  const handleUsageSubmit = (item: InventoryItem, amount: number) => {
    onUpdateStock(
      item.id,
      amount,
      'sale',  // Changed from 'usage' to 'sale'
      `Used ${amount} ${item.unit}`
    );
    setSelectedItem(null);
    setUsageAmount('');
  };
