...
export default function Header({ 
  ...
  const handleUsageSubmit = async () => {
    try {
      ...
      // Update IndexedDB
      const existingData = await getFromIndexedDB('appData');
      await saveToIndexedDB('appData', {
        ...existingData,
        inventory: updatedInventory,
        inventoryLogs: [...(existingData.inventoryLogs || []).filter((log: InventoryLog | null): log is InventoryLog => log !== null), ...updateLogs]
      });

      // Update state
      setInventoryItems(updatedInventory);
      setInventoryLogs(prev => [...(prev || []).filter((log: InventoryLog | null): log is InventoryLog => log !== null), ...updateLogs]);
