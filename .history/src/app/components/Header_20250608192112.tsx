    ...
    try {
      ...
      if (action === 'in') {
        ...
        if (!clockInResponse.ok) {
          ...
          throw new Error(error.msg || 'Failed to clock in');
        }
        ...
        const newEntry = {
          ...
          employeeName: activeEmployee,
          isSaved: true,
          _id: clockInData._id // Store the server's ID for clock-out
        }
        ...
        const clockInEntry = [...employeeTimeData].reverse()
          .find(entry => entry.employeeName === activeEmployee && entry.action === 'in' && entry._id);
        ...
        if (!clockOutResponse.ok) {
          ...
          throw new Error(error.msg || 'Failed to clock out');
        }
