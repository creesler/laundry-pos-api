  ...
  const handleUsageSubmit = async () => {
    try {
      ...
      if (soapItem && soapUsage) {
        ...
        if (usage > 0) {
          // Calculate new currentStock by adding the usage
          const newCurrentStock = Number(soapItem.currentStock) + usage
          await onUpdateInventory(
            soapItem.id, 
            newCurrentStock, // Pass the new currentStock
            'usage', 
      ...
      if (detergentItem && detergentUsage) {
        ...
        if (usage > 0) {
          // Calculate new currentStock by adding the usage
          const newCurrentStock = Number(detergentItem.currentStock) + usage
          await onUpdateInventory(
            detergentItem.id, 
            newCurrentStock, // Pass the new currentStock
            'usage', 
