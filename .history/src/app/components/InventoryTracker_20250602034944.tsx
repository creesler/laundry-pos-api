...
export default function InventoryTracker({ inventory, onUpdateStock, onAddItem, sx }: InventoryTrackerProps) {
  ...
  const handleUpdateSubmit = () => {
    if (selectedItem && updateAmount) {
      ...
      if (currentItem) {
        const newStock = updateType === 'usage' 
          ? Math.max(0, currentItem.currentStock - Number(updateAmount))
          : updateType === 'restock'
          ? Math.min(currentItem.maxStock, currentItem.currentStock + Number(updateAmount))
          : Math.min(currentItem.maxStock, Math.max(0, Number(updateAmount)));
