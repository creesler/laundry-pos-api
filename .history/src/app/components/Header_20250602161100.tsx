...
interface HeaderProps {
  ...
  setSavedData: React.Dispatch<React.SetStateAction<Array<{
    Date: string
    Coin: string
    Hopper: string
    Soap: string
  ...
  const syncWithServer = async () => {
    try {
      ...
      for (const [employeeName, entries] of entriesByEmployee) {
        ...
        const employeeResponse = await fetch(`http://localhost:5000/api/employees?name=${encodeURIComponent(employeeName)}`);
        if (!employeeResponse.ok) {
          console.error(`Failed to find employee: ${employeeName}`);
          continue;
        }
        ...
        if (!employee?._id) {
          console.error(`No ID found for employee: ${employeeName}`);
          continue;
        }
      ...
      // Now sync sales data
      const unsavedSales = savedData.filter(entry => !entry.isSaved);
      if (unsavedSales.length > 0) {
        // Format sales data for the server
        const formattedSales = unsavedSales.map(sale => ({
          date: new Date(sale.Date).toISOString(),
          coin: parseFloat(sale.Coin) || 0,
          hopper: parseFloat(sale.Hopper) || 0,
          soap: parseFloat(sale.Soap) || 0,
          vending: parseFloat(sale.Vending) || 0,
          dropOffAmount1: parseFloat(sale['Drop Off Amount 1']) || 0,
          dropOffCode: sale['Drop Off Code'] || '',
          dropOffAmount2: parseFloat(sale['Drop Off Amount 2']) || 0,
          recordedBy: activeEmployee
        }));
        ...
        const salesResponse = await fetch('http://localhost:5000/api/sales/bulk', {
          ...
          body: JSON.stringify({ entries: formattedSales })
        });
