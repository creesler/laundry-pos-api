...
export default function Header({ 
  ...
  const syncWithServer = async () => {
    try {
      ...
      for (const [employeeName, entries] of entriesByEmployee) {
        ...
        for (let i = 0; i < entries.length; i++) {
          ...
          if (entry.action === 'in') {
            ...
            if (!response.ok) {
              throw new Error(`Failed to save timesheet for ${employeeName}`);
            }
      ...
      // Update IndexedDB with saved status
      const existingData = await getFromIndexedDB() || {};
      await saveToIndexedDB({
        ...existingData,
        employeeTimeData: employeeTimeData // Now contains updated isSaved flags
      });
  ...
  // Update the onSaveToServer prop to use the new sync function
  useEffect(() => {
    if (typeof onSaveToServer === 'function') {
      onSaveToServer = syncWithServer;
    }
  }, [onSaveToServer]);
