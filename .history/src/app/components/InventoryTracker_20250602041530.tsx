  ...
  return (
    <Paper sx={{ ...sx, p: '1.5vh', display: 'flex', flexDirection: 'column', borderRadius: '8px', border: '1px solid #e5e7eb', minHeight: 0, overflow: 'hidden', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
      ...
      <Box sx={{ 
        ...
        {inventory.map((item) => {
          console.log('Rendering inventory item:', {
            id: item.id,
            name: item.name,
          ...
          return (
            ...
            >
              <Box sx={{ flex: 1 }}>
                ...
                <LinearProgress
                  variant="determinate"
                  value={item.maxStock > 0 ? (item.currentStock / item.maxStock) * 100 : 0}
                  sx={{
                    height: '1.5vh',
                    borderRadius: '4px',
                    bgcolor: grey[200],
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={handleUpdateSubmit} variant="contained">Update Stock</Button>
        </DialogActions>
      </Dialog>
