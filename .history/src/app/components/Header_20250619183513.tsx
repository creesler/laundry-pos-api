...
export default function Header({ 
  ...
  const handleUsageSubmit = async () => {
    try {
      ...
      const updateLogs = itemsToUpdate.map(({ itemId, usage }) => {
        ...
        return updateLog;
      }).filter((log): log is InventoryUpdate => log !== null);

      // Update all items in one go
      const updatedInventory = currentInventory.map((item: { id: string; currentStock: number; maxStock: number; unit: string }) => {
  ...
  const handleTimeEntry = (entry: TimeEntry) => {
    setEmployeeTimeData(prev => {
      const newEntry: TimeEntry = {
        ...entry,
        clockOutTime: entry.clockOutTime || undefined
      };
  ...
  const handleInventoryUpdate = (item: InventoryUpdate) => {
    setInventoryLogs(prev => {
      const newItem: InventoryUpdate = { ...item };
  ...
  const filterTimeEntries = (entries: TimeEntry[]): TimeEntry[] => {
    return entries.filter(entry => !entry.isSaved);
  };

  const filterInventoryUpdates = (updates: InventoryUpdate[]): InventoryUpdate[] => {
