  ...
  const handleUsageSubmit = async () => {
    try {
      ...
      if (soapItem && soapUsage) {
        ...
        if (usage > 0) {
          const newSoapStock = Math.max(0, soapItem.maxStock)
          const newSoapUsed = Number(soapItem.currentStock) + usage
      ...
      if (detergentItem && detergentUsage) {
        ...
        if (usage > 0) {
          const newDetergentStock = Math.max(0, detergentItem.maxStock)
          const newDetergentUsed = Number(detergentItem.currentStock) + usage
          await onUpdateInventory(
            detergentItem.id, 
            newDetergentStock,
            'usage', 
