  ...
  const handleUpdateSubmit = () => {
    if (selectedItem && updateAmount) {
      ...
      if (currentItem) {
        ...
        const newMaxStock = Number(maxStock) || amount; // If no max stock specified, use delivery amount
        onUpdateStock(
          selectedItem, 
          amount, // Set current stock to delivery amount
          'restock',
          `Initial stock set to ${amount}/${newMaxStock}. ${updateNotes}`
        );
      }
    }
    ...
    <Paper sx={{ ...sx, p: '1.5vh', display: 'flex', flexDirection: 'column', borderRadius: '8px', border: '1px solid #e5e7eb', minHeight: 0, overflow: 'hidden', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={handleUpdateSubmit} variant="contained">Record Delivery</Button>
        </DialogActions>
      </Dialog>
