...
export default function Header({ 
  ...
  useEffect(() => {
    ...
    window.addEventListener('appinstalled', () => {
      ...
      alert('App was successfully installed! You can now launch it from your home screen.');
    });
  }, []);
  ...
  const handleInstall = async () => {
    ...
    if (isAndroid && !deferredPrompt) {
      alert('To install on Android: tap the menu button (⋮) and select "Add to Home Screen"');
      return;
    }
    ...
    if (!deferredPrompt) {
      ...
      if (!registration) {
        alert('Service Worker is not registered yet. Please refresh the page and try again.');
        return;
      }
    ...
    try {
      ...
      alert('There was an error during installation. Please try installing from your browser\'s menu.');
    }
  };
