...
export interface HeaderProps {
  ...
  onShareClick: (_event?: React.MouseEvent) => void;
  ...
  const handleUsageSubmit = async () => {
    try {
      ...
      await saveToIndexedDB({
        ...
        inventoryLogs: [...(existingData.inventoryLogs || []).filter(log => log !== null) as InventoryLog[], ...updateLogs]
      });

      // Update state
      setInventoryItems(updatedInventory);
      setInventoryLogs(prev => [...(prev || []).filter(log => log !== null) as InventoryLog[], ...updateLogs]);
