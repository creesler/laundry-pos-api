...
export default function AdminPage() {
  ...
  const fetchEmployeeList = async () => {
    ...
    try {
      ...
      console.log('📥 Raw employee data from MongoDB:', data);
      
      // Filter only active employees
      const activeEmployees = data.filter(emp => emp.status === 'active');
      console.log('✅ Active employees:', activeEmployees);
      console.log('❌ Inactive/deleted employees:', 
        data.filter(emp => emp.status !== 'active')
      );
      
      // Compare with current list to identify changes
      const currentEmployeeNames = localEmployees.map(emp => emp.name);
      const newEmployeeNames = activeEmployees.map(emp => emp.name);
      
      const addedEmployees = newEmployeeNames.filter(
        name => !currentEmployeeNames.includes(name)
      );
      const removedEmployees = currentEmployeeNames.filter(
        name => !newEmployeeNames.includes(name)
      );
      
      console.log('➕ Newly added employees:', addedEmployees);
      console.log('➖ Removed/deactivated employees:', removedEmployees);
      
      // Update local state
      setLocalEmployees(activeEmployees);
      console.log('💾 Updated employees list:', activeEmployees);
      
      // Show user feedback for significant changes
      if (addedEmployees.length > 0) {
        setSnackbar({
          open: true,
          message: `New employee(s) added: ${addedEmployees.join(', ')}`,
          severity: 'info'
        });
      }
      if (removedEmployees.length > 0) {
        setSnackbar({
          open: true,
          message: `Employee(s) removed/deactivated: ${removedEmployees.join(', ')}`,
          severity: 'warning'
        });
      }
      ...
      console.log('✨ Employee synchronization complete!');
      ...
      setSnackbar({
        open: true,
        message: 'Error syncing employees. Falling back to cached data.',
        severity: 'error'
      });
      ...
      if (cachedEmployees) {
        ...
        console.log('📥 Found cached employees:', parsed);
  ...
  const fetchEmployeeTimesheets = async () => {
    console.log('🔄 Fetching employee timesheets...');
    try {
      const timesheets: {[key: string]: any[]} = {};
      
      // Fetch timesheets for each employee
      for (const employee of localEmployees) {
        console.log(`🔄 Fetching timesheet for ${employee.name}...`);
        
        try {
          const response = await fetch(`http://localhost:5000/api/timesheets/${employee.name}`);
          if (!response.ok) {
            throw new Error(`Failed to fetch timesheet for ${employee.name}`);
          }
          
          const data = await response.json();
          console.log(`✅ Received timesheet data for ${employee.name}:`, data);
          
          timesheets[employee.name] = data.map((entry: any) => ({
            Date: entry.date || '',
            TimeIn: entry.timeIn || '',
            TimeOut: entry.timeOut || '',
            Duration: entry.duration || '',
            Status: entry.status || '',
            EmpName: entry.employeeName || employee.name
          }));
        } catch (error) {
      ...
      console.log('💾 Setting employee timesheets:', timesheets);
  ...
  const calculateTotalHours = (employeeName: string, startDate: Date | null, endDate: Date | null) => {
    console.log('🧮 Starting total hours calculation...');
    if (!startDate || !endDate || !employeeTimesheets[employeeName]) {
      console.log('⚠️ Missing required data for calculation');
      return 0;
    }
    ...
    const totalHours = entries.reduce((total, entry) => {
      try {
        ...
        if (!isWithinInterval(entryDate, { start, end })) {
          console.log(`⏭️ Entry date ${entry.Date} outside range`);
          return total;
        }
        ...
        if (!durationMatch) {
          console.log(`⚠️ Invalid duration format: ${entry.Duration}`);
          return total;
        }
        ...
        console.log(`✅ Adding ${entryHours.toFixed(2)} hours from ${entry.Date}`);
    ...
    console.log(`💡 Final total hours: ${totalHours.toFixed(2)}`);
    return totalHours;
  };
  ...
  const handleCalculateHours = async (employeeName: string) => {
    ...
    try {
      ...
      if (!employeeData?.startDate || !employeeData?.endDate) {
        console.log('⚠️ Missing date range');
        setSnackbar({
          open: true,
      ...
      setSnackbar({
        open: true,
        message: `Hours calculated for ${employeeName}`,
        severity: 'success'
      });
  ...
  const saveToServer = async (employeeName: string) => {
    console.log('🔄 Starting saveToServer operation...');
    try {
      // First sync employees to ensure we have latest data
      console.log('📡 Syncing employee list first...');
      ...
      console.log(`🔍 Checking data for employee: ${employeeName}`);
      const employeeData = employeeHours[employeeName];
      if (!employeeData?.startDate || !employeeData?.endDate) {
        console.log('⚠️ Missing date range');
        setSnackbar({
      ...
      console.log('📊 Getting timesheet entries...');
      const entries = employeeTimesheets[employeeName] || [];
      ...
      console.log('🔍 Filtering entries within date range...');
      const timesheetEntries = entries
        ...
        .map(entry => ({
          ...
          duration: entry.Duration,
          status: entry.Status,
          employeeName: entry.EmpName || employeeName
        }));

      console.log('📝 Filtered entries:', timesheetEntries);

      // Send to server
      console.log('📡 Sending data to server...');
      const response = await fetch(`http://localhost:5000/api/timesheets/${employee._id}/bulk`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // Refresh employee list and timesheets to ensure UI is up to date
  ...
  const handleDeleteEmployee = async (employeeName: string) => {
    console.log(`🗑️ Attempting to delete employee: ${employeeName}`);
    try {
      setLoading(true);
      
      // Find employee by name
      console.log('🔍 Finding employee in database...');
      const employees = await fetchEmployeeList();
      const employeeToDelete = employees.find(emp => emp.name === employeeName);
      ...
      const response = await fetch(`http://localhost:5000/api/employees/${employeeToDelete._id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  useEffect(() => {
    if (isAuthenticated) {
      console.log('🔄 Setting up employee sync...');
      ...
      const syncInterval = setInterval(() => {
        console.log('⏰ Running periodic employee sync...');
        fetchEmployeeList();
      }, 30000);
      
      // Cleanup
      return () => {
        console.log('🧹 Cleaning up employee sync interval');
