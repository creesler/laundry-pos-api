...
export default function AdminPage() {
  ...
  const renderInventoryChart = () => {
    ...
    const chartData = inventoryData.map(item => ({
      name: item.name,
      value: inventoryChartType === 'stock' ? item.currentStock : (item.maxStock - item.currentStock),
    ...
    if (chartType === 'bar') {
      return (
        <BarChart
          ...
          series={[
            { dataKey: 'value', label: inventoryChartType === 'stock' ? 'Current Stock' : 'Used Stock' },
            { dataKey: 'maxStock', label: 'Max Stock' },
            { dataKey: 'minStock', label: 'Min Stock' }
          ]}
          height={300}
          sx={{ width: '100%' }}
        />
                ...
                <>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                    <ButtonGroup size="small">
                      <Button
                        onClick={() => setInventoryChartType('stock')}
                        variant={inventoryChartType === 'stock' ? 'contained' : 'outlined'}
                      >
                        Current Stock
                      </Button>
                      <Button
                        onClick={() => setInventoryChartType('usage')}
                        variant={inventoryChartType === 'usage' ? 'contained' : 'outlined'}
                      >
                        Stock Usage
                      </Button>
                    </ButtonGroup>
