...
export default function AdminPage() {
  ...
  const processSheet = (sheet: GoogleSheetResult): SheetData[] => {
    const rows = sheet.result.values || [];
    return rows.map(processRow);
  };

  const handleSheetData = (sheet: GoogleSheetResult) => {
    const rows = sheet.result.values || [];
    setData(rows.map(processRow));
  };

  const handleSheetProcess = (sheet: GoogleSheetResult) => {
    const rows = sheet.result.values || [];
  ...
  const handleSearch = (s: string) => {
    ...
    const filtered = data.filter(item => 
      Object.values(item).some(value => 
        value.toString().toLowerCase().includes(s.toLowerCase())
      )
    );
    setFilteredData(filtered);
  };

  const handleSheetSelect = (sheet: GoogleSheetResult) => {
    const rows = sheet.result.values || [];
    setData(rows.map(processRow));
  };
  ...
  const handleDateSearch = (s: string) => {
    if (!s.trim()) {
      setFilteredData(data);
      return;
    }
