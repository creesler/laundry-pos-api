...
export default function AdminPage() {
  ...
  const fetchEmployeeListData = async () => {
    try {
      ...
      const employeeSheets = spreadsheet.result.sheets.filter(sheet => {
        ...
        return sheetTitle && sheetTitle !== GOOGLE_SHEETS_CONFIG.RANGE.split('!')[0];
      });

      // Map sheet data to employee info with proper type checking
      ...
      console.log('Successfully fetched employee list:', employeeList);
    } catch (error: any) {
  ...
  const fetchEmployeeTimesheets = async () => {
    try {
      ...
      for (const employee of localEmployees) {
        ...
        try {
          ...
          timesheets[employee.name] = rows
            .filter(row => row && Array.isArray(row) && row.some(cell => cell && cell.trim())) // Filter out empty rows
            .map(row => ({
              Date: row[0] || '',
              TimeIn: row[1] || '',
              TimeOut: row[2] || '',
          ...
          console.log(`Successfully fetched timesheet for ${employee.name}:`, timesheets[employee.name]);
        } catch (error) {
      ...
      setSnackbar({
        open: true,
        message: error.message || 'Error loading employee timesheets',
        severity: 'error'
      });
    }
  };
  ...
  const calculateTotalHours = (employeeName: string, startDate: Date | null, endDate: Date | null) => {
    ...
    return employeeTimesheets[employeeName].reduce((total, entry) => {
      try {
        ...
        if (isWithinInterval(entryDate, { start, end })) {
          ...
          if (typeof duration === 'string' && duration !== '--') {
            ...
            if (duration.includes(':')) {
              ...
              const [hours, minutes] = duration.split(' ').filter(part => !isNaN(parseInt(part))).map(Number);
        ...
        console.error('Error calculating duration for entry:', entry, error);
        return total;
      }
    }, 0);
  };
