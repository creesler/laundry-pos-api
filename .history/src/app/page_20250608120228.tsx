  ...
  const saveToServer = async () => {
    try {
      ...
      if (selectedEmployee && !serverEmployeeNames.includes(selectedEmployee)) {
        const newSelectedEmployee = serverEmployeeNames.length > 0 ? serverEmployeeNames[0] : '';
        setSelectedEmployee(newSelectedEmployee);
        localStorage.setItem('selectedEmployee', newSelectedEmployee);
      }
      ...
      // Update IndexedDB with saved timesheet data and employee list
      await saveToIndexedDB({
        ...existingData,
        employeeTimeData: updatedTimeData,
        employeeList: serverEmployeeNames
