...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      if (savedDataStr) {
        ...
        setSavedData(data)
        // Save CSV whenever data changes
        saveLocalCsv()
      } else {
  ...
  // Update handleSave to also update CSV
  const handleSave = () => {
  ...
  // Add function to save CSV file locally
  const saveLocalCsv = () => {
    try {
      // Convert savedData to CSV using XLSX
      const worksheet = XLSX.utils.json_to_sheet(savedData)
      const csvContent = XLSX.utils.sheet_to_csv(worksheet)
      
      // Create file name with current date
      const fileName = `laundry_data_${new Date().toISOString().split('T')[0]}.csv`
      
      // Create a Blob and save it locally
      ...
      // Store the file path
      setLocalCsvPath(fileName)
    } catch (error) {
      console.error('Error saving CSV:', error)
    }
  }

  // Update email sending function to use saved CSV
