...
export default function Home() {
  ...
  const fetchTimesheetData = async () => {
    ...
    try {
      ...
      // Get employee time data from the correct property
      const employeeTimeData = data?.data?.employeeTimeData || [];
      
      // Filter for selected employee and date range
      const filteredData = employeeTimeData
        .filter((entry: any) => {
          // Filter by employee
          if (selectedEmployee && entry.employeeName !== selectedEmployee) {
            return false;
          }
          
          // Filter by date range if set
          if (timesheetDateRange.startDate && timesheetDateRange.endDate) {
      ...
      // Group entries by date and create timesheet records
        ...
        for (let i = 0; i < entries.length; i++) {
          if (entries[i].action === 'in') {
            ...
            const clockOut = entries.slice(i + 1).find(e => 
              e.action === 'out' && e.date === clockIn.date
            );
