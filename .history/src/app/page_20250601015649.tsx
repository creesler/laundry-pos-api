...
export default function Home() {
    ...
    const loadData = async () => {
      try {
        ...
        if (indexedDBData?.data) {
          ...
          const filteredData = indexedDBData.data.filter((item: SalesRecord) => {
            const itemDate = new Date(item.Date.split(' ')[0]);
            return itemDate >= oneMonthAgo;
          });
          
          // Sort data by date (newest first)
          const sortedData = filteredData.sort((a: SalesRecord, b: SalesRecord) => {
        ...
        if (isOnline) {
          ...
          if (response.result.values) {
            ...
            const filteredSheetData = sheetData.filter((item: SalesRecord) => {
              const itemDate = new Date(item.Date.split(' ')[0]);
              return itemDate >= oneMonthAgo;
            });

            // Merge with any unsaved local data
            const unsavedLocalData = savedData.filter((item: SalesRecord) => item.isSaved !== 'true').map((item: SalesRecord) => ({ ...item, isSaved: 'false' }));
            const mergedData = [...filteredSheetData, ...unsavedLocalData].sort((a: SalesRecord, b: SalesRecord) => {
              const dateA = new Date(a.Date.split(' ')[0]);
              const dateB = new Date(b.Date.split(' ')[0]);
              return dateB.getTime() - dateA.getTime();
            });
  ...
  // Update function parameters with proper types
  const handleNumpadClick = (value: string) => {
    if (selectedField) {
      setInputValues(prev => ({
        ...prev,
        [selectedField]: value === 'Del' 
          ? prev[selectedField].slice(0, -1)
          : value === 'Clr'
          ? ''
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingIndex !== null) {
        ...
        olderEntries.sort((a: SalesRecord, b: SalesRecord) => {
          const dateA = new Date(a.Date);
          const dateB = new Date(b.Date);
          return dateB.getTime() - dateA.getTime();
        });
        ...
        updatedData = [newRecord, ...todayEntries, ...olderEntries];
      }
        ...
        callback: async (tokenResponse: GoogleTokenResponse) => {
          ...
          if (tokenResponse.access_token) {
            try {
              ...
              const sortedUnsavedRecords = unsavedDailyRecords.sort((a: SalesRecord, b: SalesRecord) => {
                const dateA = new Date(a.Date.split('|')[0]);
                const dateB = new Date(b.Date.split('|')[0]);
              ...
              const updatedSavedData = savedData.map((item: SalesRecord) => 
                item.isSaved !== 'true' ? { ...item, isSaved: 'true' } : item
              );
  ...
  const handleEmployeeSelect = () => {
    if (selectedEmployee) {
      ...
      setSnackbar({
        open: true,
        message: `Selected ${selectedEmployee}`,
        severity: 'success'
      })
    }
  }
  ...
  const fetchTimesheetData = async () => {
    ...
    try {
      const data = await getFromIndexedDB();
      setTimesheetData(calculateTimesheetData(data || []));
    } catch (error) {
  ...
  // Update the timesheet data type and calculation
  ...
  const calculateTimesheetData = (records: any[]): TimesheetRecord[] => {
    return records.map(record => {
      const timeIn = new Date(record.timeIn)
      const timeOut = record.timeOut ? new Date(record.timeOut) : null
      const duration = timeOut 
        ? `${Math.floor((timeOut.getTime() - timeIn.getTime()) / (1000 * 60 * 60))}h ${Math.floor(((timeOut.getTime() - timeIn.getTime()) % (1000 * 60 * 60)) / (1000 * 60))}m`
        : 'In Progress'
      const status = timeOut ? 'Completed' : 'Pending'
  ...
  return (
    <>
      <Box sx={{
        ...
        <Box sx={{
          width: '100%',
          maxWidth: 900, // Tablet-friendly max width
          height: '100%',
          display: 'flex',
          flexDirection: 'column',
          gap: 2,
          ...
          <Header
            onShareClick={() => handleShareClick as unknown as () => void}
          ...
          <Box sx={{ display: 'flex', gap: 2, flex: 1, minHeight: 0 }}>
            <Box sx={{ flex: 3, display: 'flex', flexDirection: 'column', gap: 2, minHeight: 0 }}>
              <DailyTracker
                savedData={savedData.map((row) => ({
                  ...row,
            ...
            <Box sx={{ flex: 2, minWidth: 260, maxWidth: 350, display: 'flex', flexDirection: 'column', gap: 2, minHeight: 0 }}>
              <SalesForm
      ...
      <TimesheetModal
        ...
        timesheetData={timesheetData as TimesheetRecord[]}
      ...
      <ShareMenu
        ...
        onCloudSave={saveToGoogleSheets}
