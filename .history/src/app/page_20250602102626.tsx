...
export default function Home() {
  ...
  // Update saveToServer function to include detailed ID logging
  const saveToServer = async () => {
    try {
      ...
      // Step 1: Get all items from server first
      const getItemsResponse = await fetch('http://localhost:5000/api/inventory');
      if (!getItemsResponse.ok) {
        throw new Error('Failed to fetch items from server');
      }
      const serverItems = await getItemsResponse.json();
      
      console.log('Server items:', serverItems);
      console.log('Local items:', inventoryItems);

      // Step 2: Find items that don't exist on server (new items)
      const newItems = inventoryItems.filter(localItem => 
        !serverItems.some((serverItem: any) => 
          serverItem._id === localItem._id || 
      ...
      // Step 3: If there are new items or unsaved changes, sync with server
      ...
      if (newItems.length === 0 && 
          unsavedTimesheetEntries.length === 0 && 
          unsavedSalesEntries.length === 0 && 
          unsavedInventoryLogs.length === 0 && 
          inventoryItems.every(item => item.isSynced)) {
        setSnackbar({
          open: true,
      ...
      // Step 4: Send sync request with new items and unsaved changes
      ...
      // Step 5: Get the final list from server to get all MongoDB IDs
      const getFinalItemsResponse = await fetch('http://localhost:5000/api/inventory');
      if (!getFinalItemsResponse.ok) {
        throw new Error('Failed to fetch final items from server');
      }
      const finalServerItems = await getFinalItemsResponse.json();

      console.log('Final server items:', finalServerItems);

      // Step 6: Update local items with MongoDB IDs
      const updatedInventoryItems = inventoryItems.map(localItem => {
        const serverItem = finalServerItems.find((si: any) => 
          si.name === localItem.name || si._id === localItem._id
        );
        
        if (serverItem) {
          console.log('ID Mapping for', localItem.name, ':', {
            before: {
              localId: localItem.id,
              mongoId: localItem._id || 'None',
              isSynced: localItem.isSynced
            },
            after: {
              localId: localItem.id,
              mongoId: serverItem._id,
              isSynced: true
            }
          });
  ...
  const handleUpdateInventory = async (
    ...
    try {
      ...
      const updatedItems = inventoryItems.map(item => {
        if (item.id === itemId) {
          if (updateType === 'sale') {
            // For sales, add to currentStock (since currentStock tracks used amount)
          ...
          } else { // adjustment, damage, or other
  ...
  // Update handleAddInventoryItem to include logging
  const handleAddInventoryItem = async (newItem: Omit<InventoryItem, 'id' | 'lastUpdated'>) => {
    try {
      ...
      console.log('New inventory item created:', {
        name: newItemWithId.name,
        localId: newItemWithId.id,
        mongoId: 'Not yet assigned',
        isSynced: false
      });
      ...
      console.log('Current inventory state:', updatedItems.map(item => ({
        name: item.name,
        localId: item.id,
        mongoId: item._id || 'Not synced yet',
        isSynced: item.isSynced
      })));

      setSnackbar({
        open: true,
        message: 'New inventory item added successfully. Click "Save to Server" to sync.',
        severity: 'success'
      });
