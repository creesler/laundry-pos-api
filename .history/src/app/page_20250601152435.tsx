...
import { SnackbarState, InputValues } from './types'
...
export default function Home() {
  ...
  const handleSave = async () => {
    try {
      ...
      // Create new record with isSaved as boolean
      const newRecord = {
  ...
  const fetchTimesheetData = async () => {
    ...
    try {
      if (isOnline) {
        ...
        const response = await fetch(`http://localhost:5000/api/timesheets/${selectedEmployee}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({
            startDate: format(timesheetDateRange.startDate, 'yyyy-MM-dd'),
            endDate: format(timesheetDateRange.endDate, 'yyyy-MM-dd')
          })
        });
        ...
        if (response.ok) {
          ...
          // Merge with local data for today's entries
          const today = new Date().toISOString().split('T')[0];
          const localData = await getFromIndexedDB();
          const localTodayEntries = (localData?.employeeTimeData || [])
            .filter((entry: TimeEntry) => entry.date === today && entry.employeeName === selectedEmployee);

          // Combine server data with local today's entries
          const combinedData = [
            ...localTodayEntries,
            ...serverData.filter((entry: any) => entry.date !== today)
          ];
        ...
        const data = await getFromIndexedDB();
        const localEntries = (data?.employeeTimeData || [])
          .filter((entry: TimeEntry) => entry.employeeName === selectedEmployee)
        ...
        <Box sx={{
          ...
          <Box sx={{ 
            ...
            <Header 
              ...
              onSaveToServer={saveToServer}  // Add save to server handler
