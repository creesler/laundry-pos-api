...
// Add interface for server inventory item
...
export default function Home() {
  ...
  useEffect(() => {
    const loadInventoryData = async () => {
      try {
        ...
        if (data?.inventory) {
          console.log('Loading inventory from IndexedDB:', data.inventory);
        ...
        if (data?.inventoryLogs) {
          console.log('Loading inventory logs from IndexedDB:', data.inventoryLogs);
  ...
  // Add state for tracking deleted inventory items
  ...
  const handleDeleteInventoryItem = async (itemId: string) => {
    try {
      ...
      console.log('Adding to deletedInventoryIds:', itemId);
      setDeletedInventoryIds(prev => {
        const newDeletedIds = [...prev, itemId];
        console.log('Updated deletedInventoryIds:', newDeletedIds);
        return newDeletedIds;
      });

      // Create deletion log
      const deletionLog: InventoryUpdateLog = {
      ...
      const existingData = await getFromIndexedDB() || {};
      console.log('Current IndexedDB data:', existingData);
      
      // Save updated state to IndexedDB
      const updatedData = {
        ...existingData,
        inventory: updatedItems,
        inventoryLogs: [...(existingData.inventoryLogs || []), deletionLog],
        deletedInventoryIds: [...(existingData.deletedInventoryIds || []), itemId]
      };
      console.log('Saving to IndexedDB:', updatedData);
      
      await saveToIndexedDB(updatedData);

      // Update logs state
      setInventoryLogs(prev => [...(prev || []), deletionLog]);
      ...
      setSnackbar({
        open: true,
        message: error instanceof Error ? error.message : 'Error deleting inventory item',
        severity: 'error'
      });
    }
  };

  // Update saveToServer function to include deleted items
  const saveToServer = async () => {
    try {
      ...
      console.log('Starting saveToServer...');
      console.log('Current deletedInventoryIds:', deletedInventoryIds);
      ...
      // First, get current server inventory to map local IDs to MongoDB IDs
      console.log('Fetching current server inventory...');
      const serverInventoryResponse = await fetch('http://localhost:5000/api/inventory');
      if (!serverInventoryResponse.ok) {
        throw new Error('Failed to fetch server inventory');
      }
      const serverInventory: ServerInventoryItem[] = await serverInventoryResponse.json();
      console.log('Server inventory:', serverInventory);

      // Map local IDs to MongoDB IDs for deletion
      const serverIdsToDelete = deletedInventoryIds.map(localId => {
        // If it's already a MongoDB ID (doesn't start with local-), use it as is
        if (!localId.startsWith('local-')) {
          return localId;
        }
        // Otherwise, find the corresponding server item by name
        const localItem = inventoryItems.find(item => item.id === localId);
        if (!localItem) return null;
        
        const serverItem = serverInventory.find((item: ServerInventoryItem) => item.name === localItem.name);
        return serverItem ? serverItem._id : null;
      }).filter(Boolean); // Remove any null values

      console.log('Mapped server IDs to delete:', serverIdsToDelete);

      if (unsavedTimesheetEntries.length === 0 && 
          unsavedSalesEntries.length === 0 && 
          unsavedInventoryLogs.length === 0 &&
          serverIdsToDelete.length === 0) {
        setSnackbar({
          open: true,
      ...
      const dataToSync = {
        ...
        deletedInventoryIds: serverIdsToDelete // Use mapped server IDs
      };

      console.log('Sending data to server:', dataToSync);
      ...
      // Update local state to mark entries as saved
      const updatedTimeData = employeeTimeData.map(entry => 
      ...
      console.log('Clearing deletedInventoryIds. Current value:', deletedInventoryIds);
      ...
      // Save updated state to IndexedDB
      const updatedIndexDBData = {
        employeeTimeData: updatedTimeData,
        data: updatedSalesData,
        employeeList,
        inventory: inventoryItems,
        inventoryLogs: updatedInventoryLogs,
        deletedInventoryIds: [], // Clear deleted items
        lastSynced: new Date().toISOString()
      };
      console.log('Saving updated state to IndexedDB:', updatedIndexDBData);
      await saveToIndexedDB(updatedIndexDBData);
  ...
  const handleUpdateInventory = async (
    ...
    try {
      ...
      // Create update log with isSaved property
      const updateLog: InventoryUpdateLog = {
        id: `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        itemId,
        previousStock: currentItem.currentStock,
        newStock: updateType === 'usage' 
          ? Number(currentItem.currentStock) + Number(newStock) // For usage, add to track total used
          : Number(newStock),
        updateType,
        timestamp: new Date().toISOString(),
        updatedBy: selectedEmployee || 'unknown',
  ...
  const handleAddInventoryItem = async (newItem: Omit<InventoryItem, 'id' | 'lastUpdated'>) => {
    try {
      ...
      // Force a re-render of components by creating new array reference
      setInventoryItems([...updatedItems]);
    } catch (error) {
