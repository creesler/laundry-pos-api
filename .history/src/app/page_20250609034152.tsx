...
export default function Home() {
  ...
  const saveToServer = async () => {
    try {
      ...
      // Upload timesheet entries
      if (unsavedTimesheetEntries.length > 0) {
        console.log('⏰ Uploading timesheet entries...');
        const timesheetResponse = await fetch('http://localhost:5000/api/timesheets/bulk', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ entries: unsavedTimesheetEntries }),
        });

        if (!timesheetResponse.ok) {
          throw new Error('Failed to upload timesheet entries');
        }

        console.log('✅ Timesheet entries uploaded successfully');
      }

      // Upload sales entries
      if (unsavedSalesEntries.length > 0) {
        console.log('💰 Uploading sales entries...');
        const salesResponse = await fetch('http://localhost:5000/api/sales/bulk', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ entries: unsavedSalesEntries }),
        });

        if (!salesResponse.ok) {
          const errorData = await salesResponse.json();
          throw new Error(`Failed to upload sales entries: ${errorData.msg || salesResponse.statusText}`);
        }

        const result = await salesResponse.json();
        console.log('✅ Sales entries uploaded successfully:', result.message);
      }
