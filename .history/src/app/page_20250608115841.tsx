  ...
  const saveToServer = async () => {
    try {
      ...
      const serverEmployeeNames = serverEmployees
        ...
        .map(emp => emp.name);
      console.log('Filtered employee names:', serverEmployeeNames);
      ...
      // If current selected employee is not in the list, select the first available
      if (!serverEmployeeNames.includes(selectedEmployee) && serverEmployeeNames.length > 0) {
        setSelectedEmployee(serverEmployeeNames[0]);
        localStorage.setItem('selectedEmployee', serverEmployeeNames[0]);
      }
