...
export default function Home() {
      ...
      const tokenClient = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: async (tokenResponse: GoogleTokenResponse) => {
          ...
          try {
            ...
            // Get unsaved data
            const unsavedData = savedData.filter(entry => !entry.isSaved);
            const unsavedTimeData = employeeTimeData.filter(entry => !entry.isSaved);

            // Get current inventory state
            const currentInventory = inventoryItems;
            const existingData = await getFromIndexedDB() || {};
            const inventoryLogs = existingData.inventoryLogs || [];

            if (unsavedData.length === 0 && unsavedTimeData.length === 0 && 
                !currentInventory.length && !inventoryLogs.length) {
              setSnackbar({
                open: true,
            ...
            if (unsavedData.length > 0) {
              const salesValues = unsavedData.map(entry => [
                entry.Date,
                entry.Coin,
                entry.Hopper,
                entry.Soap,
                entry.Vending,
                entry['Drop Off Amount 1'],
                entry['Drop Off Code'],
                entry['Drop Off Amount 2']
              ]);

              const salesResult = await window.gapi.client.sheets.spreadsheets.values.append({
            ...
            // Save timesheet data
            if (unsavedTimeData.length > 0) {
              // Group entries by employee name
              const entriesByEmployee: { [key: string]: TimeEntry[] } = {};
              unsavedTimeData.forEach(entry => {
                if (!entriesByEmployee[entry.employeeName]) {
                  entriesByEmployee[entry.employeeName] = [];
                }
                entriesByEmployee[entry.employeeName].push(entry);
              });

              // Process each employee's entries
              for (const [employeeName, entries] of Object.entries(entriesByEmployee)) {
                ...
                // Create time entry pairs (in/out) for each date
                const timeEntryPairs: string[][] = [];
                Object.entries(entriesByDate).forEach(([date, dateEntries]) => {
                  ...
                  for (let i = 0; i < dateEntries.length; i++) {
                    if (dateEntries[i].action === 'in') {
                      const clockIn = dateEntries[i];
                      const clockOut = dateEntries.slice(i + 1).find(e => 
                        e.action === 'out' && e.date === dateEntries[i].date
                      );
                      ...
                      timeEntryPairs.push([
                        ...
                        clockOut ? calculateDuration(clockIn.time, clockOut.time) : '0h 0m',
                        employeeName,
                        'Regular'  // Default shift type
                      ]);

                      if (clockOut) {
                        i = dateEntries.indexOf(clockOut);
                      }
                    }
                  }
                });
                ...
                if (timeEntryPairs.length > 0) {
                  ...
                  if (timeResult.status !== 200) {
                    throw new Error(`Failed to save timesheet entries for ${employeeName}`);
                  }
                }
              }
            }
            ...
            if (currentInventory.length > 0) {
              const inventoryValues = currentInventory.map(item => [
              ...
              const inventoryResult = await window.gapi.client.sheets.spreadsheets.values.append({
                spreadsheetId: GOOGLE_SHEETS_CONFIG.SHEET_ID,
                range: 'Inventory!A:G',
                valueInputOption: 'USER_ENTERED',
                insertDataOption: 'INSERT_ROWS',
            ...
            if (inventoryLogs.length > 0) {
              const logValues = inventoryLogs.map(log => [
              ...
              const logsResult = await window.gapi.client.sheets.spreadsheets.values.append({
                spreadsheetId: GOOGLE_SHEETS_CONFIG.SHEET_ID,
                range: 'InventoryLogs!A:G',
                valueInputOption: 'USER_ENTERED',
                insertDataOption: 'INSERT_ROWS',
                resource: {
                  values: logValues
              ...
              if (logsResult.status !== 200) {
                throw new Error('Failed to save inventory logs');
              }

              // Clear inventory logs after successful save
              await saveToIndexedDB({
                ...existingData,
                inventoryLogs: []
              });
            ...
            const updatedSalesData = savedData.map(item => ({
              ...item,
              isSaved: true
            }));
  ...
  // Replace saveToGoogleSheets with saveToServer
  const saveToServer = async () => {
    try {
      ...
      // Save inventory data
      if (currentInventory.length > 0 || inventoryLogs.length > 0) {
        ...
        if (!syncResponse.ok) {
          ...
          throw new Error(errorData.message || 'Failed to sync inventory data');
        }

        // Clear inventory logs after successful sync
        await saveToIndexedDB({
          ...existingData,
          inventoryLogs: []
        });
