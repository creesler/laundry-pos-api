...
export default function Home() {
  ...
  const saveToServer = async () => {
    try {
      ...
      // Log inventory items before sync
      console.log('Inventory items before sync:', inventoryItems.map(item => ({
        name: item.name,
        localId: item.id,
      ...
      // Log server response for inventory
      console.log('Server response inventory items:', result.savedInventory?.map((item: any) => ({
        name: item.name,
        mongoId: item._id,
        frontendId: item.frontendId
      })));

      // Update inventory items with MongoDB IDs
      if (result.savedInventory) {
        const updatedInventoryItems = inventoryItems.map(item => {
          const serverItem = result.savedInventory.find(
            (si: any) => si.frontendId === item.id || si.name === item.name
          );
          if (serverItem) {
            console.log('ID Mapping for', item.name, ':', {
              before: {
                localId: item.id,
                mongoId: item._id || 'None',
                isSynced: item.isSynced
              },
              after: {
                localId: item.id,
        ...
        console.log('Final inventory state after sync:', updatedInventoryItems.map(item => ({
          name: item.name,
          localId: item.id,
          mongoId: item._id,
          isSynced: item.isSynced
